...
| 许可 | **MIT License**（详见 [LICENSE](https://github.com/unboxlumen/ndebugbar/blob/main/LICENSE)） |

## 3. 关键事实（红线，写文案时必须先确认）

1. **不是软件、不是游戏**：nDebugBar 是一个开源 Android Library，**不能直接安装运行**。它通过 `debugImplementation` 嵌入到宿主 App，宿主 Release 包不包含它。
2. **开源、MIT**：与 UnboxLumen 主线产品（N 浏览器、N 文件、N 搜索）的闭源策略不同；nDebugBar 采用宽松的 MIT License，可自由集成到闭源商用项目，只需保留版权与许可声明。
...
| 许可 | **MIT License**（宽松许可，可自由商用、闭源集成）|
...
3. **不得隐去 MIT 许可**（开源项目必须明示 License；MIT 允许商用与闭源集成，仅需保留版权与许可声明）。
